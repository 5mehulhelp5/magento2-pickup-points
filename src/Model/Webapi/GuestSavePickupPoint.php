...
use Innosend\PickupPoints\Api\Webapi\GuestSavePickupPointInterface;
...
/**
 * Save pickup point for guest cart
 */
class GuestSavePickupPoint implements GuestSavePickupPointInterface
{
    /**
    ...
    {
        // Convert masked cart ID to quote ID
        /** @var QuoteIdMask $quoteIdMask */
        $quoteIdMask = $this->quoteIdMaskFactory->create()->load($cartId, 'masked_id');
        
        if (!$quoteIdMask->getQuoteId()) {
            throw new \Magento\Framework\Exception\NoSuchEntityException(__('Cart not found'));
        }
        ...
        $this->logger->debug('Innosend Pickup Points: Saving pickup point via WebAPI (guest)', [
            'cart_id' => $cartId,
            'quote_id' => $quoteIdMask->getQuoteId(),
            'pickup_point_id' => $pickupPointData['pickup_point_id'] ?? null
        ]);

        $this->pickupPointSave->setByQuoteId($quoteIdMask->getQuoteId(), $pickupPointData);
     ...
     * @return array|null
     */
    private function normalizePickupPointData($data): ?array
    {
        if ($data === null) {
            return null;
        }
     ...
     * @return string|null
     */
    private function extractStringValue($object, string $method): ?string
    {
